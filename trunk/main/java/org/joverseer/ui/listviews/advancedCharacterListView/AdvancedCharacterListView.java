...
public class AdvancedCharacterListView extends BaseItemListView {
	...
	protected int[] columnWidths() {
		return new int[] { 96, 48, 48, 48, 48, 48, 48, 48, 48, 48, 
				32, 32, 32,
				32, 32, 32,
				120,
				48,
	...
	protected JComponent createControlImpl() {
		...
		table.setDefaultRenderer(CharacterAttributeWrapper.class,
				new CharacterAttributeWrapperTableCellRenderer(tableModel));
		table.setDefaultRenderer(ArtifactWrapper.class,
				new ArtifactWrapperTableCellRenderer(tableModel));
		return c;
	}
	...
	protected JComponent[] getButtons() {
		...
		ImageSource imgSource = (ImageSource) Application.instance()
				.getApplicationContext().getBean("imageSource");
	...
	class CopyToClipboardCommand extends ActionCommand implements ClipboardOwner {
		...
		private String getDeathReasonStr(CharacterDeathReasonEnum cd) {
			if (cd == null) return "?";
			if (cd == CharacterDeathReasonEnum.NotDead) return "";
			return cd.toString();
		}
		...
		private String getRow(AdvancedCharacterWrapper aw) {
			...
			return aw.getHexNo() + DELIM + 
					aw.getName() + DELIM +
					aw.getNationNo() + DELIM +
					nationName + DELIM + 
					getStatText(aw.getCommand()) + DELIM +
					getStatText(aw.getAgent()) + DELIM +
					getStatText(aw.getEmmisary()) + DELIM +
					getStatText(aw.getMage()) + DELIM +
					getStatText(aw.getStealth()) + DELIM +
					getStatText(aw.getChallenge()) + DELIM +
					getStatText(aw.getHealth()) + DELIM +
					getArtifactText(aw.getA0()) + DELIM +
					getArtifactText(aw.getA1()) + DELIM +
					getArtifactText(aw.getA2()) + DELIM +
					getArtifactText(aw.getA3()) + DELIM +
					getArtifactText(aw.getA4()) + DELIM +
					getArtifactText(aw.getA5()) + DELIM +
					aw.getTravellingWith() + DELIM + 
