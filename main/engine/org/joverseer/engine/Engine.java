...
public class Engine {
	...
	protected void moveArmies(Game game) {
		...
		for (MoveArmyOrder mo : moveOrders) {
			...
			for (Character charTravellingWith : ExecutingOrderUtils.getCharsWithArmy(turn, mo.getArmy(), false)) {
				charTravellingWith.setHexNo(Integer.parseInt(mo.getArmy().getHexNo()));
			}
		}
	}
	...
	protected void doExecuteOrdersPhase32(Game game) {
		doExecuteOrders(game, 319, 325);
		doExecuteOrders(game, 310, 315);
	}
	...
	protected void doExecuteOrdersPhase30(Game game) {
		doExecuteOrders(game, 270, 309);
	}

	protected void doExecuteOrdersPhase35(Game game) {
		doExecuteOrders(game, 325, 400); // TODO change this just after
		// recruiting
	}

	protected void doExecuteOrdersPhase40(Game game) {
		doExecuteOrders(game, 400, 420); // TODO change this just after
		// recruiting
	}
	...
	protected void prepareArmiesForMovement() {
		for (PopulationCenter pc : turn.getPopulationCenters()) {
			int availableFood = pc.getFoodCapacity();
			int armyRequiredFood = 0;
			for (Army a : turn.getArmies(pc.getHexNo())) {
				if (ExecutingOrderUtils.checkFriendly(turn, pc, a)) {
					armyRequiredFood += a.computeFoodConsumption();
				}
			}
			if (armyRequiredFood == 0)
				continue;
			int consumedFood = Math.min(availableFood, armyRequiredFood);
			for (Army a : turn.getArmies(pc.getHexNo())) {
				if (ExecutingOrderUtils.checkFriendly(turn, pc, a)) {
					int armyFood = a.computeFoodConsumption() * consumedFood / armyRequiredFood;
					Integer f = a.getFood();
					if (f == null)
						f = 0;
					f += armyFood;
					a.setFood(f);
					Character c = turn.getCharByName(a.getCommanderName());
					if (c != null) {
						ExecutingOrderUtils.appendMessage(c, c.getName() + "'s army received " + armyFood + " food from " + pc.getName() + ".");
					}
				}
			}
		}
		for (Army a : (ArrayList<Army>) turn.getContainer(TurnElementsEnum.Army).getItems()) {
			...
			if (armyAvailableFood == null)
				armyAvailableFood = 0;
			armyAvailableFood -= requiredFood;
			if (armyAvailableFood < 0)
				armyAvailableFood = 0;
			a.setFood(armyAvailableFood);
			boolean fed = a.getFood() > 0;
			a.setFed(fed);
			if (!fed) {
				Character c = turn.getCharByName(a.getCommanderName());
				if (c != null) {
					ExecutingOrderUtils.appendMessage(c, c.getName() + "'s army did not have enough food.");
				}
			}
