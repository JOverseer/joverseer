...
/**
 * Order editor table model Supports the display of orders in a table and the editing of the order parameters
 * 
...
public class OrderEditorTableModel extends ItemTableModel {
    ...
    protected String[] createColumnPropertyNames() {
        return new String[] {"character.nationNo", "character.name", "character.hexNo", "characterStats", "noAndCode",
        "p0", "p1", "p2", "p3", "p4",
        "p5", "p6", "p7", "p8", "p9",
        "p10", "p11", "p12", "p13", "p14",
        "draw", "results", "cost", "profit"};
    }
    ...
    protected Object getValueAtInternal(Object object, int i) {
        if (i == iNation) {
            ...
            if (game == null)
                return "";
            if (IBelongsToNation.class.isInstance(order.getCharacter())
                    && getColumnPropertyNames()[i].endsWith("nationNo")) {
            ...
            OrderResultContainer container = (OrderResultContainer) Application.instance().getApplicationContext()
                    .getBean("orderResultContainer");
            OrderResultTypeEnum orderResultType = container.getResultTypeForOrder((Order) object);
            Icon ico = null;
            if (orderResultType != null) {
                ImageSource imgSource = (ImageSource) Application.instance().getApplicationContext().getBean(
                        "imageSource");
                if (orderResultType == null) {
                    ico = null;
