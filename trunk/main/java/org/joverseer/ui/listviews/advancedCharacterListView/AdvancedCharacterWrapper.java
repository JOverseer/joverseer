...
public class AdvancedCharacterWrapper implements IHasMapLocation, IBelongsToNation, IHasTurnNumber {
	...
	public void setAttribute(CharacterAttributeWrapper value) {
		...
		if (cw != null && cw.getTurnNo() > value.getTurnNo()) return;
	...
	public static ArrayList getWrappers() {
		...
		if (!GameHolder.hasInitializedGame()) return ret.getItems();
		...
		for (int i=game.getCurrentTurn(); i>=0; i--) {
			...
			for (EnemyAgentWrapper eaw : (ArrayList<EnemyAgentWrapper>)thieves.getItems()) {
				...
				if (cw == null) {
					...
					cw.setAttributeMax(new CharacterAttributeWrapper("agent", "10", eaw.getTurnNo(), new AgentActionInfoSource(eaw.getReportedTurns())));
					ret.addItem(cw);
				} else {
					cw.setAttributeMax(new CharacterAttributeWrapper("agent", "10", eaw.getTurnNo(), new AgentActionInfoSource(eaw.getReportedTurns())));
				}
	...
	public static void guessStatsFromTitle(AdvancedCharacterWrapper cw, Character c, int turnNo) {
		...
		if (statType.equals("Commander")) {
			cw.setAttributeMax(new CharacterAttributeWrapper("command", stat, turnNo, tis));
		} else if (statType.equals("Agent")) {
			cw.setAttributeMax(new CharacterAttributeWrapper("agent", stat, turnNo, tis));
		} else if (statType.equals("Mage")) {
			cw.setAttributeMax(new CharacterAttributeWrapper("mage", stat, turnNo, tis));
		} else if (statType.equals("Emmissary")) {
			cw.setAttributeMax(new CharacterAttributeWrapper("emmisary", stat, turnNo, tis));
		};
	}
	
	public static void getStartStats(AdvancedCharacterWrapper cw) {
		Character c = (Character)GameHolder.instance().getGame().getMetadata().getCharacters().findFirstByProperty("id", cw.getId());
		if (c != null) {
	...
	public static void addStats(AdvancedCharacterWrapper cw, Character c, InfoSource is, int turnNo) {
		cw.setAttribute(new CharacterAttributeWrapper("command", c.getCommand(), turnNo, is));
		cw.setAttribute(new CharacterAttributeWrapper("commandTotal", c.getCommandTotal(), turnNo, is));
		
		cw.setAttribute(new CharacterAttributeWrapper("agent", c.getAgent(), turnNo, is));
		cw.setAttribute(new CharacterAttributeWrapper("agentTotal", c.getAgentTotal(), turnNo, is));
		
		cw.setAttribute(new CharacterAttributeWrapper("emmisary", c.getEmmisary(), turnNo, is));
		cw.setAttribute(new CharacterAttributeWrapper("emmisaryTotal", c.getEmmisaryTotal(), turnNo, is));
		
		cw.setAttribute(new CharacterAttributeWrapper("mage", c.getMage(), turnNo, is));
		cw.setAttribute(new CharacterAttributeWrapper("mageTotal", c.getMageTotal(), turnNo, is));
		
		cw.setAttribute(new CharacterAttributeWrapper("stealth", c.getStealth(), turnNo, is));
		cw.setAttribute(new CharacterAttributeWrapper("stealthTotal", c.getStealthTotal(), turnNo, is));
