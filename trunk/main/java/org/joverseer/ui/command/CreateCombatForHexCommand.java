...
public class CreateCombatForHexCommand extends ActionCommand {
    ...
    protected void doExecuteCommand() {
    	...
    	ArrayList<Army> allarmies = (ArrayList<Army>)game.getTurn().getContainer(TurnElementsEnum.Army).findAllByProperties(new String[]{"hexNo"}, new Object[]{strHex});    	
    	ArrayList<Army> fparmies = (ArrayList<Army>)game.getTurn().getContainer(TurnElementsEnum.Army).findAllByProperties(new String[]{"hexNo", "nationAllegiance"}, new Object[]{strHex, NationAllegianceEnum.FreePeople});
    	ArrayList<Army> dsarmies = (ArrayList<Army>)game.getTurn().getContainer(TurnElementsEnum.Army).findAllByProperties(new String[]{"hexNo", "nationAllegiance"}, new Object[]{strHex, NationAllegianceEnum.DarkServants});
    	ArrayList<Army> ntarmies = (ArrayList<Army>)game.getTurn().getContainer(TurnElementsEnum.Army).findAllByProperties(new String[]{"hexNo", "nationAllegiance"}, new Object[]{strHex, NationAllegianceEnum.Neutral});
    	
    	if (ntarmies.size() > 0) {
    		MessageDialog dlg = new MessageDialog("Error", "Neutral armies found in hex. Cannot create combat.");
    		dlg.showDialog();
    	}
    	...
    	if (pc != null && (pc.getNationNo() == 0 || pc.getNation().getAllegiance().equals(NationAllegianceEnum.Neutral))) {
    		MessageDialog dlg = new MessageDialog("Error", "Unknown nation or netrual pop center found in hex. Cannot create combat.");
    		dlg.showDialog();
    	}
		...
		combat.setHexNo(hex);
		String description = "Combat at " + hex;
		if (pc != null) {
