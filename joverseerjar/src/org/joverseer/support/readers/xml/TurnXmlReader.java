...
//					System.out.println(iaiName + " - " + artiNameInAscii + " " + iaiName.equalsIgnoreCase(artiNameInAscii));
	...
	public static void updateOldPCs(Turn turn,int turnNo,int tiNationNo,int nationCapitalHex,ArrayList<PopCenterWrapper> pcws,InfoSource infoSource,ArrayList<PopulationCenter> currentNationPops,GameMetadata gm) {
			...
			try {
				...
				if (oldPc == null) {
					...
					if (oldPc != null && oldPc.getInfoSource().getTurnNo() == turnNo && XmlTurnInfoSource.class.isInstance(oldPc.getInfoSource()) && ((XmlTurnInfoSource) oldPc.getInfoSource()).getNationNo() == oldPc.getNationNo()) {
						...
						if (oldPc.getTurnSeenOnMap() == turnNo) {
							...
							if (newPc.getHarbor().getSize() == 0) {
								logger.debug("keep docks from map icon");
								newPc.setHarbor(oldPc.getHarbor());
							}
						}						
		...
		ArrayList<PopulationCenter> potentiallyLostPops = turn.getPopulationCenters().findAllByProperty("nationNo", tiNationNo);
