...
public class CombatForm extends AbstractForm {
	...
	protected void refreshArmies() {
		Combat c = (Combat) getFormObject();
		ArrayList<CombatArmy> sa = new ArrayList<CombatArmy>();
		for (CombatArmy ca : c.getSide1()) {
			if (ca != null)
				sa.add(ca);
		}
		...
		ArrayList<CombatPopCenter> sp = new ArrayList<CombatPopCenter>();
		if (c.getSide2Pc() != null) {
			sp.add(c.getSide2Pc());

		}
		popCenterTableModel.setRows(sp);
