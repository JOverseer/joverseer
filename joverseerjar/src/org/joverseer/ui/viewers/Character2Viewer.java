 ...
 */
public class Character2Viewer extends ObjectViewer {
	...
	public Character2Viewer(FormModel formModel) {
	...
	protected JComponent createFormControl() {
		...
		scene = new JFXPanel();
		scene.setPreferredSize(this.uiSizes.newDimension(160/12, this.uiSizes.getHeight3()));
		...
		this.characterName.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				TransferHandler handler = Character2Viewer.this.characterName.getTransferHandler();
				handler.exportAsDrag(Character2Viewer.this.characterName, e, TransferHandler.COPY);
			}
		});
		c.setBorder(border);
		...
		btnCharDetails.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				Character2Viewer.this.showArtifacts = !Character2Viewer.this.showOrders;
				Character2Viewer.this.showSpells = !Character2Viewer.this.showOrders;
				Character2Viewer.this.showHostages = !Character2Viewer.this.showOrders;
				Character2Viewer.this.showOrders = !Character2Viewer.this.showOrders;
		...
		this.artifactsTable.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if (e.getClickCount() == 1) {
					if (Character2Viewer.this.artifactsTable.getSelectedRow() < 0)
						return;
					ArtifactInfo a = (ArtifactInfo) tableModel.getRow(Character2Viewer.this.artifactsTable.getSelectedRow());
					if (a == null)
						return;
					TransferHandler handler = new ArtifactInfoExportTransferHandler(a);
					Character2Viewer.this.artifactsTable.setTransferHandler(handler);
					handler.exportAsDrag(Character2Viewer.this.artifactsTable, e, TransferHandler.COPY);
		...
		this.spellsTable.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if (e.getClickCount() == 1) {
					if (Character2Viewer.this.spellsTable.getSelectedRow() < 0)
						return;
					SpellProficiency sp = (SpellProficiency) spellModel.getRow(Character2Viewer.this.spellsTable.getSelectedRow());
					if (sp == null)
						return;
					TransferHandler handler = new ParamTransferHandler(sp.getSpellId());
					Character2Viewer.this.spellsTable.setTransferHandler(handler);
					handler.exportAsDrag(Character2Viewer.this.spellsTable, e, TransferHandler.COPY);
				} else if (e.getClickCount() == 2 && e.getButton() == MouseEvent.BUTTON1) {
					if (Character2Viewer.this.spellsTable.getSelectedRow() < 0)
						return;
					SpellProficiency sp = (SpellProficiency) spellModel.getRow(Character2Viewer.this.spellsTable.getSelectedRow());
		...
		panel.setFocusTraversalPolicy(new FocusTraversalPolicy() {
			...
			public Component getComponentAfter(java.awt.Container aContainer, Component aComponent) {
				if (aComponent == Character2Viewer.this.characterName) {
					return btnMainMenu;
				} else if (aComponent == btnMainMenu) {
					return btnCharDetails;
				} else if (aComponent == btnCharDetails) {
					return Character2Viewer.this.order1comp.getFocusTraversalPolicy().getFirstComponent(Character2Viewer.this.order1comp);
				} else if (aComponent == Character2Viewer.this.order1comp.getFocusTraversalPolicy().getLastComponent(Character2Viewer.this.order1comp)) {
					return Character2Viewer.this.order2comp.getFocusTraversalPolicy().getFirstComponent(Character2Viewer.this.order2comp);
				} else if (aComponent == Character2Viewer.this.order2comp.getFocusTraversalPolicy().getLastComponent(Character2Viewer.this.order1comp)) {
					return Character2Viewer.this.characterName;
				}
				return Character2Viewer.this.characterName;
			}
			...
			public Component getComponentBefore(java.awt.Container aContainer, Component aComponent) {
				if (aComponent == Character2Viewer.this.characterName) {
					return Character2Viewer.this.order2comp.getFocusTraversalPolicy().getLastComponent(Character2Viewer.this.order2comp);
				} else if (aComponent == btnMainMenu) {
					return Character2Viewer.this.characterName;
				} else if (aComponent == btnCharDetails) {
					return btnMainMenu;
				} else if (aComponent == Character2Viewer.this.order1comp.getFocusTraversalPolicy().getFirstComponent(Character2Viewer.this.order1comp)) {
					return btnCharDetails;
				} else if (aComponent == Character2Viewer.this.order2comp.getFocusTraversalPolicy().getFirstComponent(Character2Viewer.this.order2comp)) {
					return Character2Viewer.this.order1comp.getFocusTraversalPolicy().getLastComponent(Character2Viewer.this.order1comp);
				}
				return Character2Viewer.this.characterName;
			}
			...
			public Component getLastComponent(java.awt.Container aContainer) {
				return Character2Viewer.this.order2comp.isVisible() ? Character2Viewer.this.order2comp.getFocusTraversalPolicy().getLastComponent(Character2Viewer.this.order2comp) : btnCharDetails;
			}
	...
	private class ShowArtifactsCommand extends ActionCommand {
		...
		protected void doExecuteCommand() {
			Character2Viewer.this.showArtifacts = !Character2Viewer.this.showArtifacts;
			refresh();
		}
	}
	...
	private class ShowSpellsCommand extends ActionCommand {
		...
		@Override
		protected void doExecuteCommand() {
			Character2Viewer.this.showSpells = !Character2Viewer.this.showSpells;
			refresh();
		}
	}
		...
		protected void doExecuteCommand() {
			...
			for (int i = 0; i < c.getNumberOfOrders(); i++) {
				if (c.getOrders()[i].isBlank()) {
					...
					setFormObject(getFormObject());
					Character2Viewer.this.showOrders = true;
