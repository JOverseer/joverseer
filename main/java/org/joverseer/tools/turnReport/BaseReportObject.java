...
public class BaseReportObject implements IHasMapLocation, Comparable {
	String name;
	...
	public void setNationNo(Integer nationNo) {
		if (new Integer(this.nationNo).equals(nationNo)) return;
	...
	public void setNations(ArrayList nations) {
	...
	public void removeNation(int nationNo) {
		if (this.nationNo == nationNo) this.nationNo = 0;
		if (nations.contains(nationNo)) nations.remove(nationNo);
	}
	
	public void addNation(int nationNo) {
		if (nationNo == 0) return;
		if (!nations.contains(nationNo)) nations.add(nationNo);
		if (this.nationNo == 0) this.nationNo = nationNo;
	}
	...
	public String getNationsStr() {
		String ret = "";
		for (int nationNo : (ArrayList<Integer>)getNations()) {
			if (nationNo == 0) continue;
	...
	public int compareTo(Object o) {
		if (BaseReportObject.class.isInstance(o)) {
			...
			if (getNationNo() == 0) return 1;
			if (ro.getNationNo() == 0) return -1;
	...
	public String getHtmlString() {
		...
		//appendTd(getModification()) +
			appendTd(getNationsStr()) +
				...
				appendTd(getNotes()) +
				appendTd(getExtraInfo()) +
				appendTd(getLinks());
		
	}
	
	public String getHexNoStr() {
		if (getHexNo() == 0) return "";
		String c = String.valueOf(getHexNo());
		if (getHexNo() < 1000) c = "0" + c;
		return c;
	}
			...
			if (ps[0].equals("hex")) {
				...
				if (hexNo != 0) {
					...
					Application.instance().getApplicationContext().publishEvent(
				                        new JOverseerEvent(LifecycleEventsEnum.SelectedHexChangedEvent.toString(), p, null));
				}
				...
				Character c = t.getCharById(charId);
				if (c != null) challenge = t.findChallenge(c.getName());
				if (challenge == null) {
