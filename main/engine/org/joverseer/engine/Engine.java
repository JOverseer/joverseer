...
import org.joverseer.domain.IMaintenanceCost;
...
import org.joverseer.ui.listviews.CombatCalcCombatTableModel;
...
public class Engine {
	...
	public void executeOrders(Game game) {
		try {
			...
			runCombats();
			initializeEconomies1();
			doExecuteOrdersPhase30(game);
			initializeEconomies2();
			produceProducts();
			doExecuteOrdersPhase35(game);
	...
	protected void initializeTurn() {
		...
		for (Character c : turn.getAllCharacters()) {
			if (c.isDead()) deadCharacters.add(c);
		}
		turn.getContainer(TurnElementsEnum.Character).removeAll(deadCharacters);
	}
	...
	protected void moveArmies(Game game) {
		...
		sittingArmies.addAll((ArrayList<Army>)turn.getContainer(TurnElementsEnum.Army).getItems());
		...
		// we don't need to check against pop centers, this has been taken care of by MoveArmyOrder.prepareMoves, 
						...
						for (MoveArmyOrder nmo : moveOrders) {
							...
							if (nmo.getCurrentHex() == mo.getCurrentHex()) {
								...
								if (!ExecutingOrderUtils.checkNonHostile(turn, a, b)) {
									if (d2 == d1) {
										// nmo has already moved, so this is an intercept
										...
										{
											System.out.println("Intercept between " + mo.getName() + " and " + nmo.getName());
											// nmo does change hex this day or moves opposite to mo
											mo.stopBecauseOfIntercept();
											nmo.stopBecauseOfIntercept();
										}
	...
	protected void doExecuteOrdersPhase35(Game game) {
		doExecuteOrders(game, 325, 390); //TODO change this just after recruiting
	}
	
	protected void doExecuteOrdersPhase40(Game game) {
		doExecuteOrders(game, 390, 420); //TODO change this just after recruiting
	}
	...
	protected void prepareArmiesForMovement() {
		for (Army a : (ArrayList<Army>)turn.getContainer(TurnElementsEnum.Army).getItems()) {
			...
			if (requiredFood == null) requiredFood = 0;
			...
			Character ac = turn.getCharByName(a.getCommanderName());
			if (ac != null && popUsedFood > 0) {
				ExecutingOrderUtils.appendMessage(ac, ac.getName() + "'s army received " + popUsedFood + " food from " + pc.getName() + ".");
			}
				
			Integer armyAvailableFood = a.getFood();
			if (armyAvailableFood == null) armyAvailableFood = 0;
			
			int armyUsedFood = Math.min(armyAvailableFood, requiredFood - popUsedFood);
			if (ac != null && armyUsedFood > 0) {
				ExecutingOrderUtils.appendMessage(ac, ac.getName() + "'s army consumed " + armyUsedFood + " from it's baggage.");
			}
			a.setFood(armyAvailableFood - armyUsedFood);
			a.setFed(a.getFood() > 0);
			if (ac != null && a.isFed() == false) {
				ExecutingOrderUtils.appendMessage(ac, ac.getName() + "'s army is unfed.");
			}
	...
	protected void initializeEconomies1() {
		for (NationEconomy ne : (ArrayList<NationEconomy>)turn.getContainer(TurnElementsEnum.NationEconomy).getItems()) {
			ne.setAvailableGold(ne.getReserve());
			ne.setReserve(0);
		}
	}
	
	protected void initializeEconomies2() {
		for (NationEconomy ne : (ArrayList<NationEconomy>)turn.getContainer(TurnElementsEnum.NationEconomy).getItems()) {
	...
	protected int computeArmyMaintenanceCosts(NationEconomy ne) {
		int cost = 0;
		ArrayList<Army> as = (ArrayList<Army>)turn.getContainer(TurnElementsEnum.Army).findAllByProperty("nationNo", ne.getNationNo()); 
		for (Army i : as) {
	...
	protected void postprocessArmies() {
		for (Army a : (ArrayList<Army>)turn.getContainer(TurnElementsEnum.Army).getItems()) {
			...
			sortArmyElements(a);
			Character c = ExecutingOrderUtils.getCommander(turn, a, false);
			if (c != null) a.setCommanderTitle(c.getTitle());
	...
	protected void prepareTurn(Game game) {
		...
		turn.getContainer(TurnElementsEnum.Encounter).clear();
		turn.getContainer(TurnElementsEnum.NationMessage).clear();
		turn.getContainer(TurnElementsEnum.ArmyEstimate).clear();
		try {
			game.addTurn(turn);
		}
		catch (Exception exc) {
