		...
		mapPanel.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent arg0) {
				if (arg0.getKeyCode() == KeyEvent.VK_UP) {
					scp.getVerticalScrollBar().setValue(scp.getVerticalScrollBar().getValue() - scp.getVerticalScrollBar().getBlockIncrement());
				}
				if (arg0.getKeyCode() == KeyEvent.VK_DOWN) {
					scp.getVerticalScrollBar().setValue(scp.getVerticalScrollBar().getValue() + scp.getVerticalScrollBar().getBlockIncrement());
				}
				if (arg0.getKeyCode() == KeyEvent.VK_LEFT) {
					scp.getHorizontalScrollBar().setValue(scp.getHorizontalScrollBar().getValue() - scp.getHorizontalScrollBar().getBlockIncrement());
				}
	...
	public void onApplicationEvent(ApplicationEvent applicationEvent) {
		if (applicationEvent instanceof JOverseerEvent) {
			...
			if (e.getEventType().equals(LifecycleEventsEnum.GameChangedEvent.toString())) {
				...
				MapMetadata mm = (MapMetadata) Application.instance().getApplicationContext().getBean("mapMetadata");
				mapPanel.setPreferredSize(new Dimension(mm.getGridCellWidth() * mm.getHexSize() * (mm.getMaxMapColumn() + 1), mm.getGridCellHeight() * mm.getHexSize() * mm.getMaxMapRow()));
				scp.getVerticalScrollBar().setUnitIncrement(mm.getGridCellHeight() * mm.getHexSize() * 2);
				scp.getHorizontalScrollBar().setUnitIncrement(mm.getGridCellWidth() * mm.getHexSize() * 2);
